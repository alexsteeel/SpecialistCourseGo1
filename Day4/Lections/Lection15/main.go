...
func main() {
	...
	fmt.Println("bigFunction:", bigFunction(30, 40))
	...
	// 4. Тип функции
	fmt.Printf("Тип функции: %T\n", res)
	fmt.Printf("Тип функции: %T\n", calcAndReturnValidFunc)

	fmt.Println("result with receiveFuncAndReturnValue add is", receiveFuncAndReturnValue(add))
	fmt.Println("result with receiveFuncAndReturnValue res is", receiveFuncAndReturnValue(res))
	fmt.Println("result with receiveFuncAndReturnValue random is", receiveFuncAndReturnValue(func(a, b int) int { return a*a + b*b + 2 }))
}
...
// поэтому ее можно присваивать переменной, передавать в качестве параметра и возвращать в качестве результата
