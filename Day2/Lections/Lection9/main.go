...
func main() {
	...
	// Нужно обратиться к элементу массива по индексу
	// arr[i] = elem
	arr[0] = 10
	arr[1] = 20
	arr[3] = -100
	arr[4] = 1000
	fmt.Println("This is my updated array:", arr)

	// 3. Определение массива с указанием элементов на месте.
	// Если при инциализации кол-во элементов меньше номинальной длины массива,
	// то недостающие элементы инициализируются значениями по умолчанию.
	newArr := [5]int{10, 20, 30}
	fmt.Println(newArr)

	// 4. Создание массива через инициализацию переменных.
	arrWithValues := [...]int{10, 20, 30, 40}
	fmt.Println("Array declaration with values", arrWithValues, len(arrWithValues))
	...
	// 6. Массив и его размер - это две составляющие одного типа
	// var third [2]int
	// third = first
